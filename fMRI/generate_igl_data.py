...
def main():
    ...
    patterns_perception = np.load('data/patterns_perception.npy')
    patterns_imagination = np.load('data/patterns_imagination.npy')
    patterns_judgment = np.load('data/patterns_judgment.npy')
    ...
    noise_perception = fmrisim.generate_noise(dimensions=dim[0:3],
                                tr_duration=int(tr),
                                stimfunction_tr=[0] * dim[3],
                                mask=mask,
                                template=template,
    ...
    noise_judgment = fmrisim.generate_noise(dimensions=dim[0:3],
                                tr_duration=int(tr),
                                stimfunction_tr=[0] * dim[3],
                                mask=mask,
                                template=template,
    ...
    That is: preception and judgement are both in posterior parietal cortex. not sure about the imagination. 
    """
    ...
    onsets_perception = [[] for _ in range(num_class)]
    length = len(onsets_all)
    data_action = []
    for i in range(length):
        onsets_imagination[np.random.randint(num_class)].append(onsets_all[i])
        action = np.random.randint(num_class)
        onsets_perception[action].append(onsets_all[i])
        data_action.append(action)

    for i in range(num_class):
        onsets_imagination[i] = np.array(onsets_imagination[i])
        onsets_perception[i] = np.array(onsets_perception[i])

    # determin match/mismatch
    onsets_judgment = [[] for _ in range(2)]

    data_reward = []
    for i in range(num_class):
        for event in onsets_imagination[i]:
            if event in onsets_perception[i]: # Ture means perception=imagination for this event
                onsets_judgment[1].append(event)
                data_reward.append(1)
            else:
                onsets_judgment[0].append(event)
                data_reward.append(0)

    for i in range(2):
        onsets_judgment[i] = np.array(onsets_judgment[i]) + 2

    for i in range(num_class):
        onsets_perception[i] += 2

    onsets_imagination_all = onsets_all.copy()
    onsets_perception_all = onsets_all + 2
    onsets_judgment_all = onsets_all + 2
    ...
    stimfunc_list_perception = create_multiple_events(onsets_perception)
    stimfunc_list_judgment = create_multiple_events(onsets_judgment)
    ...
    stimfunc_weighted_perception = compute_weighted_stimfunc(stimfunc_list_perception,patterns_perception)
    stimfunc_weighted_judgment = compute_weighted_stimfunc(stimfunc_list_judgment,patterns_judgment)
    ...
    signal_func_perception = fmrisim.convolve_hrf(stimfunction=stimfunc_weighted_perception,
                                    tr_duration=tr,
                                    temporal_resolution=temporal_res,
                                    scale_function=1,
                                    )

    signal_func_judgment = fmrisim.convolve_hrf(stimfunction=stimfunc_weighted_judgment,
                                    tr_duration=tr,
                                    temporal_resolution=temporal_res,
                                    scale_function=1,
                                    )
    ...
    noise_func_perception = noise_perception[signal_idxs[0], signal_idxs[1], signal_idxs[2], :].T
    noise_func_judgment = noise_judgment[signal_idxs[0], signal_idxs[1], signal_idxs[2], :].T
    ...
    signal_func_scaled_perception = fmrisim.compute_signal_change(signal_func_perception,
                                                    noise_func_perception,
                                                    noise_dict,
                                                    magnitude=signal_magnitude,
                                                    method=signal_method,
                                                    )

    signal_func_scaled_judgment = fmrisim.compute_signal_change(signal_func_judgment,
                                                    noise_func_judgment,
                                                    noise_dict,
                                                    magnitude=signal_magnitude,
                                                    method=signal_method,
    ...
    signal_perception = fmrisim.apply_signal(signal_func_scaled_perception,
                                signal_volume,
                                )

    signal_judgment = fmrisim.apply_signal(signal_func_scaled_judgment,
                                signal_volume,
                                )
    ...
    brain_perception = signal_perception + noise_perception * noise_weight
    brain_judgment = signal_judgment + noise_judgment * noise_weight
    ...
    data_perception = brain_perception[lb[0]:ub[0], lb[1]:ub[1], lb[2]:ub[2], ((onsets_perception_all + hrf_lag) / tr).astype('int')]
    data_perception= data_perception.reshape((voxels, data_perception.shape[3]))
    ...
    data_judgment = brain_judgment[lb[0]:ub[0], lb[1]:ub[1], lb[2]:ub[2], ((onsets_judgment_all + hrf_lag) / tr).astype('int')]
    data_judgment= data_judgment.reshape((voxels, data_judgment.shape[3]))

    data_feedback = 0.5 * (data_perception + data_judgment)

    np.save('data/data_imagination_seed_' + str(args.seed) + '_noise_level_' + str(args.noise_weight) +  '.npy', data_imagination.T)
    np.save('data/data_action_seed_' + str(args.seed) + '_noise_level_' + str(args.noise_weight) + '.npy', data_action)
    np.save('data/data_feedback_seed_' + str(args.seed) + '_noise_level_' + str(args.noise_weight) + '.npy', data_feedback.T)
    np.save('data/data_reward_seed_' + str(args.seed) + '_noise_level_' + str(args.noise_weight) + '.npy', data_reward)
